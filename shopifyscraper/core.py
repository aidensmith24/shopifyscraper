...
class ShopifyScraper:
    ...
    Supports proxy rotation, analysis summaries, and professional PDF reporting.
    """
    ...
    def is_definitely_shopify(self, url: str) -> bool:
        ...
        try:
            ...
            if any("shopify" in h.lower() for h in r.headers.keys()):
                return True
        except Exception:
            pass
        try:
            ...
            html = r.text.lower()
            if "cdn.shopify.com" in html or "shopify-digital-wallet" in html:
                return True
        except Exception:
            pass
        return False
    ...
    def tag_summary(self, top_n: int = 10) -> Dict[str, int]:
        ...
        return dict(tags.most_common(top_n))
        ...
        for p in self.products_cache:
            for v in p.get("variants", []):
                ...
                if v.get("available", False):
                    in_stock += 1
        return {"total_variants": total, "in_stock": in_stock, "out_of_stock": total - in_stock}
    ...
    def avg_price_by(self, field: str = "vendor") -> pd.Series:
        ...
        return pd.Series(avg_prices).sort_values(ascending=False)
    ...
    def keyword_summary(self, top_n:int=15) -> Dict[str,int]:
        ...
        words = Counter()
        for t in titles:
            words.update([w for w in re.findall(r'\w+', t) if len(w)>2])
        return dict(words.most_common(top_n))

    # -------------------------------
    # PDF report generation
    # -------------------------------
    def generate_report(self, filename: str = "shopify_report.pdf", export_csv: bool = True):
        ...
        story.append(Paragraph("Shopify Store Analysis Report", title_style))
        story.append(Spacer(1, 12))
        story.append(Paragraph(f"Store: {store_name}", styles['Heading2']))
        ...
        if not avg_vendor.empty:
            ...
            plt.figure(figsize=(6,3))
            avg_vendor.plot(kind="bar", color="lightgreen", edgecolor="black")
        ...
        for p in self.products_cache:
            for v in p.get("variants", []):
                ...
                if available and 0 < qty <= low_stock_threshold:
                    low_stock_items.append([p.get("title","Unknown"), v.get("title",""), qty])
        story.append(Paragraph("<b>Low Stock Alerts</b>", styles["Heading2"]))
        if low_stock_items:
            tdata = [["Product","Variant","Qty"]]+low_stock_items
            t=Table(tdata,colWidths=[200,150,50])
            ...
            story.append(Paragraph("No low stock items.", styles["Normal"]))
        story.append(Spacer(1,18))
        ...
        if tags:
            tdata = [["Tag","Count"]]+[[k,v] for k,v in tags.items()]
            t=Table(tdata,colWidths=[250,150])
            t.setStyle(TableStyle([
                ("BOX",(0,0),(-1,-1),1,colors.black),
                ("INNERGRID",(0,0),(-1,-1),0.5,colors.grey)
            ]))
        ...
        if keywords:
            kdata = [["Keyword","Count"]]+[[k,v] for k,v in keywords.items()]
            ktable=Table(kdata,colWidths=[250,150])
        ...
        # Optional CSV export
        if export_csv:
            df_products = pd.json_normalize(self.products_cache)
            csv_file = filename.replace(".pdf", ".csv")
            df_products.to_csv(csv_file, index=False)
            print(f"[INFO] CSV export generated: {csv_file}")
